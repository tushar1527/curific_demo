...
const findAllQuery = async (query) => {
	...
	const data = await packagesModel
		.find(whereClause)
		.skip(page > 0 ? +limit * (+page - 1) : 0)
		.limit(+limit || 20)
		.sort(sort);
	const totalCount = await packagesModel.find(whereClause).countDocuments();
...
const updateOneQuery = async (filter, update, projection) => {
	...
	const data = await packagesModel.findOneAndUpdate(filter, update, options);
	return data;
};
